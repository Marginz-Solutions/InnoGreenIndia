...
import type { Brand, Category, PaginationType } from '@/lib/global.types';
...
export function BrandSection({
  ...
  const fetchBrands = useCallback(
    async (reset: boolean, currentPage: number) => {
  ...
  const handleDelete = async (id: string) => {
    if (
      !confirm(
        'Caution: Delete this brand?, If you delete this brand, you lost all the things related to this brand!',
      )
    )
      return;
    try {
    ...
    <>
      <div className="flex flex-wrap items-start justify-between gap-3 mb-5">
        <div>
          ...
          <p className="muted text-sm">
            Manage brand partnerships and logos displayed on the website
          </p>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <span className="ml-auto text-xs text-[#9bb4a1] font-medium">
            {pagination.loading && items.length === 0
              ? 'Loading…'
              : `${pagination.total} brand${pagination.total !== 1 ? 's' : ''}`}
          </span>
        </div>
      </div>
      ...
      {!pagination.loading && items.length === 0 ? (
        <EmptyState
          icon={Briefcase}
          title="No brands yet"
          desc="Add your first brand partner to feature it on the website"
          onAdd={openAdd}
        />
      ) : (
        <>
                ...
                <div className="flex items-center gap-4 flex-1">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-1">
                      <h3
                        className="text-lg! font-bold text-[#102018] truncate"
                        title={b.name}
                      >
                        {b.name}
                      </h3>
                ...
                {b.categories.length > 0 && (
                  <div className="flex flex-wrap gap-1">
                    {b.categories.map((c) => (
                      <span
                        key={c.id}
                        className="px-2 py-0.5 bg-[#edf8ee] text-[#1f7a36] text-xs font-semibold rounded-lg border border-[#c8e6cc]"
                      >
                        {c.name}
                      </span>
                    ))}
