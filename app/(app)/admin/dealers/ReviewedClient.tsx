...
export default function ReviewedDealersClient({
  ...
  const categories = useMemo(
    () => [...new Set(dealers.map((d) => d.categoryInterest))].sort(),
    [dealers]
  );
  ...
  const filtered = useMemo(() => {
    return dealers?.filter((d) => {
      ...
      const matchQuery =
        ...
        d?.categoryInterest?.toLowerCase().includes(q) ||
        d?.gstNumber?.toLowerCase().includes(q);

      const matchCategory =
        filters.category === "all" || d.categoryInterest === filters.category;
      ...
      const matchVolume =
        filters.volume === "all" ||
        (filters.volume === "provided" && !!d.monthlyVolume) ||
        (filters.volume === "not_provided" && !d.monthlyVolume);
          ...
          <div className="sm:flex border border-[#cfe0d2] rounded-xl overflow-hidden">
            <button
              ...
              className={`px-3 py-2.5 transition-colors ${
                view === "table" ? "bg-[#edf8ee] text-[#1f7a36]" : "text-[#9bb4a1] hover:text-[#61756a]"
              }`}
            ...
            <button
              ...
              className={`px-3 py-2.5 border-l border-[#cfe0d2] transition-colors ${
                view === "grid" ? "bg-[#edf8ee] text-[#1f7a36]" : "text-[#9bb4a1] hover:text-[#61756a]"
              }`}
